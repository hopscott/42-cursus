...
t_cmdl	*ft_cmdlnew(char *cmd, char **env)
{
	t_cmdl	*cmdl;
	t_cmdl	**args;
	...
	if (!cmdl)
		return (0);
	cmdl->cmd = ft_strcpy(cmd);
	cmd_pathfinder(&cmdl->cmd, env);
	cmdl->args = malloc(sizeof(char *) * 2);
	cmdl->args[0] = ft_strcpy(cmdl->cmd);
	cmdl->args[1] = NULL;
	cmdl->next = 0;
	return (cmdl);
}	
...
int	main(int ac, char **av, char **env)
{
	...
	*lst = NULL;
	...
	while (i < ac)
	{
		...
		{
			cmdl = ft_cmdlnew(ac[i], env);
			if (*lst == NULL)
				*lst = cmdl;
			else
				ft_lstadd_back(lst, cmdl);
		}
		i++;
	}

	execute_pipes(lst, env);
