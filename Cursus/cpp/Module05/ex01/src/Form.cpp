...
/*   Updated: 2022/11/18 20:25:45 by swillis          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				Form::signForm(const Bureaucrat & bcrat)
{
	...
	if (bcrat.getGrade() < HIGHEST_GRADE)
	{
		std::cout << bcrat.getName() << " couldn't sign form " << *this << " because grade too high!" << std::endl;
		throw GradeTooHighException();
	}
	else if (bcrat.getGrade() > _grade_to_sign)
	{
		std::cout << bcrat.getName() << " couldn't sign form " << *this << " because grade too low!" << std::endl;
		throw GradeTooLowException();
	}
	...
	if (_is_signed == false)
	{
		...
		std::cout << bcrat.getName() << " couldn't sign form " << *this << " because already signed!" << std::endl;
